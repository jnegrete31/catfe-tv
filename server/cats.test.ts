...
// ============ ADOPTED IN LOUNGE STATUS & BULK UPDATE ============
...
describe("cats.bulkUpdateStatus (admin)", () => {
  ...
  it("accepts all valid status values including adopted_in_lounge", async () => {
...
describe("cats create/update with adopted_in_lounge", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("can create a cat with adopted_in_lounge status", async () => {
    const mockCat = {
      ...
      adoptedDate: new Date("2026-02-19"),
      adoptedBy: "Jane Doe",
      isFeatured: false,
      sortOrder: 0,
      createdAt: new Date(),
    ...
    expect(result.adoptedDate).toEqual(new Date("2026-02-19"));
  });

  it("can update a cat to adopted_in_lounge status", async () => {
    const mockCat = {
      id: 5,
      name: "Luna",
      breed: "Siamese",
      ...
      vaccinationsDue: null,
      fleaTreatmentDue: null,
      adoptedDate: new Date("2026-02-18"),
      adoptedBy: "John Smith",
      isFeatured: false,
      sortOrder: 0,
      createdAt: new Date(),
      updatedAt: new Date(),
    };
    vi.mocked(db.updateCat).mockResolvedValue(mockCat);
